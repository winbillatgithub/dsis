...
<body class="no-skin">
	...
	<script type="text/javascript">
		...
		function sendFhsms(username){
			 ...
			 diag.Drag=true;
			 diag.Title ="站内信";
			 diag.URL = '/fhsms/goAdd.do?username='+username;
			 diag.Width = 660;
			 diag.Height = 444;
		...
		function del(ztid,STATUS,type,Id,SANME_ID){
			bootbox.confirm("确定要删除吗?", function(result) {
				...
				if(result) {
					if (typeof top.jzts === "function") {top.jzts();}
					var url = "/fhsms/delete.do?FHSMS_ID="+Id+"&tm="+new Date().getTime();
		...
		function viewx(ztid,STATUS,type,Id,SANME_ID){
			if(type == "1" && STATUS == '2' && $("#"+ztid).html() == '<span class="label label-important arrowed-in">未读</span>'){
			 ...
			 diag.Drag=true;
			 diag.Title ="站内信";
			 diag.URL = '/fhsms/goView.do?FHSMS_ID='+Id+'&TYPE='+type+'&SANME_ID='+SANME_ID+'&STATUS='+STATUS;
			 diag.Width = 600;
			 diag.Height = 460;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '/fhsms/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		function viewUser(USERNAME){
			if('admin' == USERNAME){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '/user/view.do?USERNAME='+USERNAME;
			 diag.Width = 469;
			 diag.Height = 380;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
