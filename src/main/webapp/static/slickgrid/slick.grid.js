...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    var currentEditorForSearch = null;	// added by winbill for search
    var serializedEditorValue;
    var editController;
    var editControllerForSearch;		// added by winbill for search
    ...
    function init() {
      ...
      // added by winbill for search
      editControllerForSearch = {
        "commitCurrentEditForSearch": commitCurrentEditForSearch,
        "cancelCurrentEditForSearch": cancelCurrentEditForSearch
      };
      $container
          .empty()
          .css("overflow", "hidden")
    ...
    function finishInitialization() {
     ...
     // winbill, for ios bug: resizeCanvas() called infinite loops
//      $container
//          .on("resize.slickgrid", resizeCanvas);
    ...
    // added by winbill for search
    function getEditControllerForSearch() {
    	return editControllerForSearch;
    }
        ...
        if (!e.shiftKey && !e.altKey && !e.ctrlKey) {
          ...
          if (e.which == keyCode.ESCAPE) {
            ...
            if (options.editable) {
              if (currentEditor) {
                // adding new row
            	// Modified by winbill, navigate to the next editable cell
//                if (activeRow === getDataLength()) {
//                  navigateDown();
//                } else {
//                  commitEditAndSetFocus();
//                }
            	handled = navigateNextEdit();
            	if (handled == false) {
            		navigateDown();
            	}
            	// Modified by winbill, End.
              } else {
                if (getEditorLock().commitCurrentEdit()) {
    ...
    // added by winbill for search
    function makeActiveCellNormalForSearch() {
      if (!currentEditorForSearch) {
        return;
      }
      trigger(self.onBeforeCellEditorDestroy, {editor: currentEditorForSearch, grid: self});
      currentEditorForSearch.destroy();
      currentEditorForSearch = null;
...
//      }
      ...
      getEditorLock().deactivate(editControllerForSearch);
    }
    ...
    // for searching, winbill
    function makeActiveCellEditableForSearch(editor) {

      // cancel pending async call if there is one
      clearTimeout(h_editorLoader);

      var columnDef = columns[0];
      var item = getDataItem(0);

      getEditorLock().activate(editController);
      $(activeCellNode).addClass("editable");

	  var useEditor = editor || getEditor(0, 0);
      ...
      var cell = getCellNode(0, 0);
      currentEditorForSearch = new useEditor({
        grid: self,
        gridPosition: absBox($container[0]),
        position: absBox(cell),
        container: cell,
        column: columnDef,
        item: item || {},
        commitChanges: commitEditAndSetFocus,
        ...
        initCallback: function() {
          if (item) {
            currentEditorForSearch.loadValue(item);
          }

          serializedEditorValue = currentEditorForSearch.serializeValue();

          if (currentEditorForSearch.position) {
            handleActiveCellPositionChange();
          }
        }
      });
      ...
      if (item) {
//        currentEditorForSearch.loadValue(item);  // do NOT need load the first row's data
      }
...
//      }
    ...
    // added by winbill.
    function getCellEditorForSearch() {
    	return currentEditorForSearch;
    }
    ...
    function findFirstEditableCell(row) {
      var cell = 0;
      while (cell < columns.length) {
        if (canCellBeEdited(row, cell)) {
          return cell;
        }
        cell += getColspan(row, cell);
      }
    ...
    function gotoRightEdit(row, cell, posX) {
      if (cell >= columns.length) {
        return null;
      }
      ...
      while (cell < columns.length && !canCellBeEdited(row, cell));
    ...
    function gotoNextEdit(row, cell, posX) {
      if (row == null && cell == null) {
        row = cell = posX = 0;
        if (canCellBeEdited(row, cell)) {
          return {
      ...
      var pos = gotoRightEdit(row, cell, posX);
      if (pos) {
        return pos;
      }
      ...
      var dataLengthIncludingAddNew = getDataLengthIncludingAddNew();
      while (++row < dataLengthIncludingAddNew) {
        firstFocusableCell = findFirstEditableCell(row);
        if (firstFocusableCell !== null) {
          return {
            "row": row,
            "cell": firstFocusableCell,
    ...
    function navigateNextEdit() {
      return navigate("nextEdit");
    }
    ...
    function canCellBeEdited(row, cell) {
      if (row >= getDataLength() || row < 0 || cell >= columns.length || cell < 0) {
        return false;
      }

      return columns[cell].editor != null;
    }
    ...
    // added by winbill for search
    function commitCurrentEditForSearch() {
    	return true;
    }
    function cancelCurrentEditForSearch() {
    	makeActiveCellNormalForSearch();
    	return true;
    }
    ...
    $.extend(this, {
      ...
      "updateColumnOptions": updateColumnOptions, // added by winbill
      ...
      "editActiveCellForSearch": makeActiveCellEditableForSearch,
      "getCellEditor": getCellEditor,
      "getCellEditorForSearch": getCellEditorForSearch,	// added by winbill for searching panel
      ...
      "getEditControllerForSearch": getEditControllerForSearch
    });
