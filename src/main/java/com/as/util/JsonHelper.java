...
public class JsonHelper {
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> T jsonToBean(String jsonString, JsonConfig jc) {

        JSONObject jsonObject = JSONObject.fromObject(jsonString);
        T bean = (T) JSONObject.toBean(jsonObject, jc);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static String beanListToDataGridJson(int listCout, List beans) {

		StringBuffer rest = new StringBuffer();
		int size = beans.size();
		rest.append("{");
		rest.append("\"total\":\"" + listCout + "\",");
		rest.append("\"rows\":[");
		for (int i = 0; i < size; i++) {
			rest.append(beanToJson(beans.get(i)) + ((i < size - 1) ? "," : ""));
	...
	public static String mapToJson(Map<String, ?> map) {
		 ...
		 * String s_json = "{";
		 * 
		 * Set<String> key = map.keySet(); for (Iterator<?> it = key.iterator();
		 * it.hasNext();) { String s = (String) it.next(); if (map.get(s) ==
		 * null) {
		 * 
		 * } else if (map.get(s) instanceof List<?>) { s_json += (s + ":" +
		 * JsonHelper.beanListToJson((List<?>) map.get(s)));
		 * 
		 * } else { JSONObject json = JSONObject.fromObject(map); s_json += (s +
		 * ":" + json.toString());
		 * 
		 * }
		 * 
		 * if (it.hasNext()) { s_json += ","; } }
		 * 
		 * s_json += "}"; return s_json;
		 */
	 ...
	 * @return
	 */
	public static String mapToJson(Map<String, ?> map, String dateFormat) {
	...
	public static String mapToJson(Map<String, ?> map, String[] _nory_changes, boolean nory) {
		...
		for (Iterator<?> it = key.iterator(); it.hasNext();) {
			...
			if (map.get(s) == null) {
				...
				s_json += (s + ":" + JsonHelper.beanListToJson((List<?>) map.get(s), _nory_changes, nory));
	...
	public static <T> List<T> jsonToBeanList(String jsonString, Class<T> beanClass) {
		...
		for (int i = 0; i < size; i++) {
			...
			String[] dateFormats = new String[] {"yyyy-MM-dd HH:mm:ss","yyyy-MM-dd","yyyy-MM","yyyy"};
			JSONUtils.getMorpherRegistry().registerMorpher(new DateMorpherHelper(dateFormats,(Date) null)); 
