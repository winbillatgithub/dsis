...
<body class="no-skin">
	...
	<script type="text/javascript">
		...
		if (typeof(top.hangge) === "function") {if (typeof top.hangge === "function") {$(top.hangge());}}
		//检索
		function search(){
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					if (typeof top.jzts === "function") {top.jzts();}
					var url = "/textmsg/delete.do?TEXTMSG_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '/textmsg/goEdit.do?TEXTMSG_ID='+Id;
			 diag.Width = 600;
			 diag.Height = 360;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '/textmsg/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='/textmsg/excel.do';
		};
		</script>
