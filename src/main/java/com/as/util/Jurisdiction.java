		...
		for(int i=0;i<menuList.size();i++){
			if(menuList.get(i).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0])){ //访问地址与菜单地址循环匹配，如何匹配到就进一步验证，如果没匹配到就不处理(可能是接口链接或其它链接)
				if(!menuList.get(i).isHasMenu()){				//判断有无此菜单权限
					...
					// Fixed bug: can't retrieve value when elements removed
//					map.remove("add");
//					map.remove("del");
//					map.remove("edit");
//					map.remove("cha");
					...
					map.put("add", (RightsHelper.testRights(map.get("add"), MENU_ID)) || isAdmin?"1":"0");
					map.put("del", RightsHelper.testRights(map.get("del"), MENU_ID) || isAdmin?"1":"0");
					map.put("edit", RightsHelper.testRights(map.get("edit"), MENU_ID) || isAdmin?"1":"0");
					map.put("cha", RightsHelper.testRights(map.get("cha"), MENU_ID) || isAdmin?"1":"0");
	...
	public static String getUsername(){
		return String.valueOf(getSession().getAttribute(Const.SESSION_USERNAME));
	}
	 ...
	 * @return
	 */
	public static String getTopCorpId(){
		User user = (User) Jurisdiction.getSession().getAttribute(Const.SESSION_USER);
        if (user != null) {
            return user.getTOP_CORP_ID();
        }
        return null;
	}
	 ...
	 * @return
	 */
	public static String getTopCorpBianma(){
		User user = (User) Jurisdiction.getSession().getAttribute(Const.SESSION_USER);
		return user.getTOP_CORP_BIANMA();
	}
