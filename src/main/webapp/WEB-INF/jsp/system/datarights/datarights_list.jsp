			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="/datarights/list.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center" style="display:none">角色ID</th>
									<th class="center">角色名称</th>
									<th class="center">表名</th>
									<th class="center">权限</th>
									<th class="center">操作符</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center' style="display:none">${var.ROLE_ID}</td>
											<td class='center'>
												<c:forEach items="${roleList}" var="role">
													<c:if test="${role.ROLE_ID == var.ROLE_ID }">${role.ROLE_NAME }</c:if>
												</c:forEach>
											</td>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<c:if test="${pd.roleId != null && pd.roleId != '' }">
									<a class="btn btn-sm btn-success" onclick="cancel('${pd.roleId}');">取消</a>
									</c:if>
								</td>
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					if (typeof top.jzts === "function") {top.jzts();}
					var url = "/datarights/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		//取消
		function cancel(ROLE_ID) {
			if (typeof top.jzts === "function") {top.jzts();}
			window.location.href="/role.do";//?ROLE_ID="+ROLE_ID;
		}
		//修改
		function edit(Id){
			 if (typeof top.jzts === "function") {top.jzts();}
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '/datarights/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '/datarights/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='/datarights/excel.do';
		}
