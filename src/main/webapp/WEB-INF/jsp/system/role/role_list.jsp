							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="margin-top:7px;">
								...
								<c:choose>
									<c:when test="${not empty roleList_z}">
										...
										<a class="btn btn-mini btn-purple" onclick="editDataRights('${var.ROLE_ID }');"><i class="icon-pencil"></i>数据权限</a>
	...
	<script type="text/javascript">
		...
		function editRights(ROLE_ID){
			if (typeof top.jzts === "function") {top.jzts();}
			 ...
			 diag.Drag = true;
			 diag.Title = "菜单权限";
			 diag.URL = '/role/menuqx.do?ROLE_ID='+ROLE_ID;
			 diag.Width = 320;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function editDataRights(ROLE_ID) {
			if (typeof top.jzts === "function") {top.jzts();}
			window.location.href="/datarights/list.do?roleId="+ROLE_ID;
		}
		//按钮权限(增删改查)
		function roleButton(ROLE_ID,msg){
			...
			if(msg == 'add_qx'){
			 ...
			 diag.Drag = true;
			 diag.Title = Title;
			 diag.URL = '/role/b4Button.do?ROLE_ID='+ROLE_ID+'&msg='+msg;
			 diag.Width = 330;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
