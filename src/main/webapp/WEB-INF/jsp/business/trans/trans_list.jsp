			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="/trans/list.do" method="post" name="Form" id="Form">
										...
										<tr>
											...
											<td class='center'>
												<c:forEach items="${deptList}" var="item">
													<c:if test="${item.CORP_ID == var.CORP_ID }">${item.NAME }</c:if>
												</c:forEach>
											</td>
											<c:set var="data_date" value="${fn:substring(var.DATA_DATE, 0, 10)}" />
											<td class='center'>${data_date}</td>
											...
											<td class='center'>${var.AUDIT_LEVEL}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="审核" onclick="edit('${var.TRANS_ID}','${var.APPROVE_FLAG}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.TRANS_ID}','${var.APPROVE_FLAG}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					if (typeof top.jzts === "function") {top.jzts();}
					var url = "/trans/delete.do?TRANS_ID="+Id+"&tm="+new Date().getTime();
		...
		//修改
		function edit(Id, APPROVE_FLAG){
			 if (typeof top.jzts === "function") {top.jzts();}
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '/trans/goEdit.do?TRANS_ID='+Id+'&APPROVE_FLAG='+APPROVE_FLAG;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '/trans/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='/trans/excel.do';
		}
	</script>
