...
*/var swfobject = function() {
    ...
    function s(Y) {
        if (typeof O.addEventListener != D) O.addEventListener("load", Y, !1); else if (typeof j.addEventListener != D) j.addEventListener("load", Y, !1); else if (typeof O.attachEvent != D) i(O, "onload", Y); else if (typeof O.onload == "function") {
            var X = O.onload;
            O.onload = function() {
                X(), Y();
            };
    ...
    function H() {
        ...
        if (ag > 0) for (var af = 0; af < ag; af++) {
            ...
            if (M.pv[0] > 0) {
                var ae = c(Y);
                if (ae) if (F(o[af].swfVersion) && !(M.wk && M.wk < 312)) w(Y, !0), ab && (aa.success = !0, aa.ref = z(Y), ab(aa)); else if (o[af].expressInstall && A()) {
                    var ai = {};
                    ...
                    var ah = {}, X = ae.getElementsByTagName("param"), ac = X.length;
    ...
    function z(aa) {
        var X = null, Y = c(aa);
        if (Y && Y.nodeName == "OBJECT") if (typeof Y.SetVariable != D) X = Y; else {
...
SWFUpload == undefined && (SWFUpload = function(a) {
    ...
    var c = window.location.protocol + "//" + window.location.hostname + (window.location.port ? ":" + window.location.port : ""), b = window.location.pathname.lastIndexOf("/");
    ...
    var b = this.buildParamString(), a = this.settings.http_success.join(",");
    ...
    var c = this.settings.post_params, b = [];
    if (typeof c == "object") for (var a in c) c.hasOwnProperty(a) && b.push(encodeURIComponent(a.toString()) + "=" + encodeURIComponent(c[a].toString()));
    return b.join("&amp;");
    ...
    var e = /[$]([0-9a-f]{4})/i, f = {}, d;
    if (c != undefined) {
        for (var a in c.post) if (c.post.hasOwnProperty(a)) {
            d = a;
            var b;
    ...
    var methods = {
        "init": function(options, swfUploadOptions) {
            return this.each(function() {
                var $this = $(this), $clone = $this.clone(), settings = $.extend({
                    "id": $this.attr("id"),
                    "swf": "uploadify.swf",
                ...
                var playerVersion = swfobject.getFlashPlayerVersion(), flashInstalled = playerVersion.major >= 9;
                if (flashInstalled) {
        ...
        "cancel": function(fileID, supressEvent) {
            ...
            this.each(function() {
                ...
                } else for (var n = 0; n < args.length; n++) swfuploadify.cancelUpload(args[n]), $("#" + args[n]).find(".data").removeClass("data").html(" - Cancelled"), $("#" + args[n]).find(".uploadify-progress-bar").remove(), $("#" + args[n]).delay(1e3 + 100 * n).fadeOut(500, function() {
                    $(this).remove();
                }); else {
        ...
        "settings": function(name, value, resetObjects) {
            var args = arguments, returnValue = value;
            this.each(function() {
                var $this = $(this), swfuploadify = $this.data("uploadify"), settings = swfuploadify.settings;
                if (typeof args[0] == "object") for (var n in value) setData(n, value[n]);
                if (args.length === 1) returnValue = settings[name]; else {
                    switch (name) {
                      case "uploader":
        ...
        "upload": function() {
            ...
            this.each(function() {
                ...
                if (args[0]) if (args[0] == "*") swfuploadify.queueData.uploadSize = swfuploadify.queueData.queueSize, swfuploadify.queueData.uploadQueue.push("*"), swfuploadify.startUpload(); else {
        ...
        "onSelect": function(file) {
            var settings = this.settings, queuedFile = {};
            ...
            var fileSize = Math.round(file.size / 1024), suffix = "KB";
        ...
        "onUploadComplete": function(file) {
            var settings = this.settings, swfuploadify = this, stats = this.getStats();
            ...
            if ($.inArray("onUploadComplete", settings.overrideEvents) < 0) if (settings.removeCompleted) switch (file.filestatus) {
        ...
        "onUploadError": function(file, errorCode, errorMsg) {
            var settings = this.settings, errorString = "Error";
            switch (errorCode) {
        ...
        "onUploadProgress": function(file, fileBytesLoaded, fileTotalBytes) {
            var settings = this.settings, timer = new Date, newTime = timer.getTime(), lapsedTime = newTime - this.timer;
            lapsedTime > 500 && (this.timer = newTime);
            var lapsedBytes = fileBytesLoaded - this.bytesLoaded;
            this.bytesLoaded = fileBytesLoaded;
            var queueBytesLoaded = this.queueData.queueBytesUploaded + fileBytesLoaded, percentage = Math.round(fileBytesLoaded / fileTotalBytes * 100), suffix = "KB/s", mbs = 0, kbs = lapsedBytes / 1024 / (lapsedTime / 1e3);
        ...
        "onUploadStart": function(file) {
            var settings = this.settings, timer = new Date;
        ...
        "onUploadSuccess": function(file, data, response) {
            var settings = this.settings, stats = this.getStats();
