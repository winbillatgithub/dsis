...
function intModel(initdata)
{
	...
	//if(sessionStorage.getItem("flowsheet")){
		...
		for(var key in flowsheet.nodes){  
            ...
            var modelIndex = $(mode).attr("modelIndex");
            var left = $(mode).attr("left"); // 位置
            var top = $(mode).attr("top"); // 
            mainHTML+='<div class="model" id="' + id + '" modelCode="' + code + '" modelType="' + modelType +'"modelIndex="' + modelIndex +'">' + getModelElementStr(id,code,modelType) + '</div>';
...
function parseProperties(pid,obj,tableName)
{
	...
	for(var i=0;i<obj.length;i++){
		...
		if(colProperty.colCode != undefined){
			str += '<li id=' + pid + '_' + colProperty.colCode +  '  tabName=' + tableName +   '  colCode=' + colProperty.colCode +  '  colIndex=' + colProperty.colIndex + '>' 
				+ colProperty.des + '</li>';
		}else{
			bootbox.alert("属性解析错误.");
			str += "";
		}
	}
	return str;
...
function removeElement(obj)
{
    ...
    bootbox.confirm("确定移除该表单？", function(result) {
        if(result) {
