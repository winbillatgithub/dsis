						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center" style="display:none">公司ID</th>
									<th class="center">公司名称</th>
									<th class="center">装置名称</th>
									<th class="center">装置类型</th>
									<th class="center">处理能力</th>
									<th class="center">投产时间</th>
									<th class="center">工艺情况</th>
									<th class="center">控制系统类型</th>
									<th class="center">DCS_PLC厂商</th>
									<th class="center">排序码</th>
									<th class="center">停用标志</th>
										...
										<tr>
											...
											<td class='center'>
												<c:forEach items="${departmentList}" var="department">
													<c:if test="${department.CORP_ID == var.CORP_ID }">${department.NAME }</c:if>
												</c:forEach>
											</td>
											<td class='center'>${var.NAME}</td>
											<td class='center'>${var.CATEGORY}</td>
											<td class='center'>${var.CAPACITY}</td>
											<td class='center'>${var.PRODUCT_TIME}</td>
											<td class='center'>${var.ART}</td>
											<td class='center'>${var.CS_KIND}</td>
											<td class='center'>${var.DCS_PRODUCER}</td>
											<td class='center'>${var.SORT_NO}</td>
											<td class='center'>
												<c:if test="${var.STOP == 0}">
												<font color="blue">未停用</font>
												</c:if>
												<c:if test="${var.STOP == 1}">
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					if (typeof top.jzts === "function") {top.jzts();}
					var url = "/device/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '/device/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '/device/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='/device/excel.do';
		}
	</script>
