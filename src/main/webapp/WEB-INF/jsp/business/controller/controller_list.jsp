			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="/controller/list.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">系统ID</th>
									<th class="center">父控制器ID</th>
									<th class="center">名称</th>
									<th class="center">功能描述</th>
									<th class="center">投用状态位号</th>
									<th class="center">投用状态位号数值</th>
									<th class="center">子控制器标志</th>
									<th class="center">停用原因类型</th>
									<th class="center">停用详情</th>
									<th class="center">记录停用标志</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.SYSTEM_ID}</td>
											<td class='center'>${var.PARENT_ID}</td>
											<td class='center'>${var.NAME}</td>
											<td class='center'>${var.FUNCTION_DESC}</td>
											<td class='center'>${var.POSITIONS}</td>
											<td class='center'>${var.POSITIONS_VALUE}</td>
											<td class='center'>${var.HAS_CHILDREN}</td>
											<td class='center'>${var.STOP_TYPE}</td>
											<td class='center'>${var.STOP_DETAIL}</td>
											<td class='center'>${var.STOP}</td>
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					if (typeof top.jzts === "function") {top.jzts();}
					var url = "/controller/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '/controller/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '/controller/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='/controller/excel.do';
		}
	</script>
