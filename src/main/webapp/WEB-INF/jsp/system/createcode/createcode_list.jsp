...
<body class="no-skin">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center' style="width: 30px;">${vs.index+1}</td>
													...
													<td class="center">${fn:replace(var.TABLENAME, ',as,', '')}</td>
	...
	<script type="text/javascript">
		...
		function productCode(CREATECODE_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="代码生成器";
			 diag.URL = '/createCode/goProductCode.do?CREATECODE_ID='+CREATECODE_ID;
			 diag.Width = 800;
			 diag.Height = 450;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					if (typeof top.jzts === "function") {top.jzts();}
					var url = "/createCode/delete.do?CREATECODE_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '/createcode/goEdit.do?CREATECODE_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type:"POST",
								url: '/createCode/deleteAll.do?tm='+new Date().getTime(),
						    	data:{DATA_IDS:str},
								dataType:'json',
